...
$CurrentValues = Get-SleepAfter

if ($CurrentValues.AC -ne $ACValue){
    Set-SleepAfter -PowerSource AC -Minutes $ACValue
}

if ($CurrentValues.DC -ne $DCValue){
    Set-SleepAfter -PowerSource Battery -Minutes $DCValue
}
